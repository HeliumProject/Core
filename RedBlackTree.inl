    ...
    size_t nodeIndex ) const
{
    ...
    if( nodeIndex >= m_values.GetSize() )
    {
        HELIUM_TRACE(
            TraceLevels::Debug,
            "Invalid RedBlackTree node index %" PRIuSZ " (max: %" PRIuSZ ").\n",
            nodeIndex,
            m_values.GetSize() );
    ...
    if( !m_blackNodes[ nodeIndex ] )
    {
        if( IsValid( child0Index ) && !m_blackNodes[ child0Index ] )
        {
            HELIUM_TRACE(
                TraceLevels::Debug,
                "Consecutive red nodes found in RedBlackTree (nodes %" PRIuSZ " and %" PRIuSZ ").\n",
                nodeIndex,
                child0Index );
    ...
    if( IsValid( child0Index ) )
    {
        ...
        if( !keyCompare( rChildKey, rNodeKey ) )
        {
            HELIUM_TRACE(
                TraceLevels::Debug,
                "RedBlackTree sort mismatch between node %" PRIuSZ " and child node %" PRIuSZ ".\n",
                nodeIndex,
                child0Index );
    ...
    if( child0BlackNodeCount != child1BlackNodeCount )
    {
        HELIUM_TRACE(
            TraceLevels::Debug,
            "RedBlackTree black node depth mismatch between subtrees at nodes %" PRIuSZ " (%" PRIuSZ " black nodes) and %" PRIuSZ " (%" PRIuSZ " black nodes).\n",
            child0Index,
