...
int64_t StaticMemoryStream::Seek( int64_t offset, SeekOrigin origin )
{
    ...
    switch( origin )
    {
        ...
        default:
        {
            HELIUM_TRACE( TraceLevels::Error, "DynamicMemoryStream::Seek(): Invalid seek origin specified.\n" );
    ...
    if( offset < 0 )
    {
        ...
        if( absOffset > static_cast< uint64_t >( referenceOffset ) )
        {
            HELIUM_TRACE(
                TraceLevels::Error,
                "DynamicMemoryStream::Seek(): Attempted to seek before the start of the memory stream.\n" );
        }
        else
        {
    ...
    else
    {
        ...
        if( absOffset > static_cast< uint64_t >( static_cast< size_t >( m_pEnd - m_pStart ) - referenceOffset ) )
        {
            HELIUM_TRACE(
                TraceLevels::Error,
                "DynamicMemoryStream::Seek(): Attempted to seek past the end of the memory stream.\n" );
        }
        else
        {
            m_pCurrent = m_pStart + referenceOffset + static_cast< size_t >( absOffset );
...
int64_t DynamicMemoryStream::Seek( int64_t offset, SeekOrigin origin )
{
    ...
    else
    {
        ...
        if( absOffset >= static_cast< uint64_t >( static_cast< size_t >( -1 ) - referenceOffset ) )
        {
            HELIUM_TRACE(
                TraceLevels::Error,
                "DynamicMemoryStream::Seek(): Attempted to seek outside the maximum buffer size supported by the current platform.\n" );
        }
        else
        {
            m_offset = referenceOffset + static_cast< size_t >( absOffset );
