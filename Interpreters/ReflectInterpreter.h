...
namespace Helium
{
    namespace Inspect
    {
        ...
        public:
            ...
            typedef std::map< const Reflect::MetaClass*, V_Creator > M_Creator;

            static void Register(const Reflect::MetaClass* type, uint32_t mask, Creator creator);

            template< class T >
            static void Register(const Reflect::MetaClass* type, uint32_t mask = 0x0)
            {
                Register( type, mask, &CreateInterpreter<T> );
            }

            static void Unregister(const Reflect::MetaClass* type, uint32_t mask, Creator creator);

            template< class T >
            static void Unregister(const Reflect::MetaClass* type, uint32_t mask = 0x0)
            {
                Unregister( type, mask, &CreateInterpreter<T> );
            }

            static ReflectFieldInterpreterPtr Create( const Reflect::MetaClass* type, uint32_t flags, Container* container);
            static void Clear();
