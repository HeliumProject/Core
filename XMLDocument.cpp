...
bool XMLDocument::Parse( const void* buffer, uint32_t lengthInBytes, bool finalize )
{
	return XML_STATUS_OK == XML_Parse(m_Parser, static_cast< const char* >( buffer ), lengthInBytes, finalize);
}
...
bool XMLDocument::Save( const char* file )
{
	return true;
}
